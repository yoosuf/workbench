...
export const AddRelationshipModal: React.FC<AddRelationshipModalProps> = ({
    ...
    try {
      if (relKind === 'MANY_TO_MANY') {
        ...
        if (res.data?.createTable) {
          // Link Junction Table to Source Table
          ...
          // Link Junction Table to Referenced Table
        ...
        // Standard 1:N, 1:1, or Identifying Container
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="bg-[#161b22] border border-[#30363d] rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden text-[#c9d1d9] font-sans">
        {/* Header */}
        <div className="px-5 py-4 border-b border-[#30363d] flex items-center justify-between bg-[#0d1117]">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Workbench Relationship Designer</h3>
              <p className="text-[11px] text-[#8b949e]">
                Model 1:N, 1:1, N:M Many-to-Many, and Identifying Container dependencies
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs font-sans">
          {errorMsg && (
            <div className="p-3 rounded-lg bg-[#2b0f14]/50 border border-[#f85149]/40 text-[#ff7b72] flex items-start space-x-2 font-mono">
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <span>{errorMsg}</span>
            </div>
          )}

          {/* Relationship Cardinality Cards (Workbench Style) */}
          <div>
            <label className="block text-[#8b949e] font-semibold mb-2 uppercase text-[10px] tracking-wider">
              Select Relationship Type (Cardinality)
            </label>
            <div className="grid grid-cols-2 gap-2.5">
              ...
              >
                ...
                <p className="text-[10px] text-[#8b949e] leading-snug">
                  Standard foreign key. Child references Parent (Non-Identifying dashed connector).
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center space-x-1.5 font-bold font-mono text-xs text-[#3fb950]">
                    ...
                    <span>1 : N Identifying Container</span>
                  </div>
                  {relKind === 'IDENTIFYING_CONTAINER' && <Check className="w-3.5 h-3.5 text-[#3fb950]" />}
                </div>
                <p className="text-[10px] text-[#8b949e] leading-snug">
                  Child table lifecycle is contained in Parent table. Strict Cascade Delete (Solid connector).
                </p>
              </button>
              ...
              >
                ...
                <p className="text-[10px] text-[#8b949e] leading-snug">
                  Unique relationship. Exactly one record in Table A matches one record in Table B.
                </p>
              </button>
              ...
              >
                ...
                <p className="text-[10px] text-[#8b949e] leading-snug">
                  Auto-generates a Junction / Bridge table with dual foreign key constraints.
                </p>
              </button>
            </div>
          </div>
          ...
          {/* Conditional Fields depending on RelKind */}
          {relKind === 'MANY_TO_MANY' ? (
            <div>
              <label className="block text-[#8b949e] font-semibold mb-1">
              ...
              <input
                ...
                className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-3 py-2 text-white font-mono focus:outline-none focus:border-[#d29922]"
                required
              />
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-3 py-2 text-white font-mono focus:outline-none focus:border-[#58a6ff] disabled:opacity-60"
                  >
          ...
          <div className="p-3 rounded-xl bg-[#0d1117] border border-[#30363d] flex items-center justify-between font-mono text-xs">
          ...
          <div className="pt-3 border-t border-[#30363d] flex items-center justify-end space-x-2">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 py-2 rounded-lg bg-[#21262d] hover:bg-[#30363d] text-white text-xs font-medium transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-lg bg-[#1f6feb] hover:bg-[#388bfd] text-white text-xs font-semibold shadow-sm transition-all disabled:opacity-50"
            >
              {loading ? (
                <>
